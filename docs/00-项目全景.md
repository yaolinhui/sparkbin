...
| `DotGridBackground.tsx` | 登录/着陆页背景网格动效：Canvas 2D + 弹簧物理模型，鼠标移动产生斥力涟漪，移开弹性回位，支持暗色/亮色主题 |
