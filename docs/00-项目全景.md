...
- **商业模式**：**开源核心 + 社区会员增值**。核心功能开源（Elastic License 2.0），自托管无限制；Pro/Team 会员解锁高级功能。Elastic License 防止云厂商未经授权将 SparkBin 作为托管服务提供，保护项目可持续性
...
| 国际化 | 自建 i18n（7 语言：中/日/韩/西/法/德/英） |
| 支付 | Stripe Test Mode |
...
| 容器化 | Docker + Docker Compose（开发/生产双配置） |
| 本地 AI | Ollama（OpenAI 兼容 API，无需 API Key，完全离线） |
...
├── fix-ssl.sh                        # SSL 证书快速配置脚本（Let's Encrypt + Nginx）
├── .github/                          # GitHub 仓库配置
│   ├── workflows/
│   │   └── deploy.yml                # CI/CD：master 分支推送自动部署到阿里云（Docker）
│   ├── PULL_REQUEST_TEMPLATE.md      # PR 模板（含文档更新检查清单）
│   └── ISSUE_TEMPLATE/               # Issue 模板（Bug / Feature）
├── docker/                           # Docker 容器化配置
│   ├── Dockerfile.backend            # 后端多阶段构建
│   ├── Dockerfile.frontend           # 前端构建 + Nginx
│   └── nginx.conf                    # 反向代理配置
├── docker-compose.yml                # 开发环境编排（SQLite + 可选 Ollama）
├── docker-compose.production.yml     # 生产环境编排（PostgreSQL + Nginx）
├── .env.example                      # 环境变量完整模板
├── docs/                             # 项目文档中心
│   ├── 00-项目全景.md                 # ← 本文件
│   ├── 01-架构决策记录.md             # 关键架构决策（DECISIONS，DEC-001 ~ DEC-027）
│   ├── 02-文档更新规则.md             # 文档治理规则
│   ├── 03-Agent规则.md               # AI 助手系统指令与代码规范
│   ├── 04-任务边界约束模板.md          # 任务执行边界条件模板
│   └── tasks/                        # 具体任务记录
│       └── YYYY-MM-DD-任务名称.md
├── landing/                          # Next.js 独立宣传网站
│   ├── app/                          # App Router（多语言路由）
│   ├── components/                   # 页面区块组件
│   ├── i18n/                         # 落地站国际化（zh/en）
│   └── styles/                       # 全局样式
├── frontend/                         # React 前端项目（主应用）
│   ├── src/
...
| 文件 | 功能 |
|------|------|
| `ProjectBoard.tsx` | 项目仪表盘（首页）：终端风格 Header、Metrics Bar（总项目/活跃/暂停/归档筛选）、项目网格（ProjectCard）、归档折叠面板、右下角 PixelPet 像素宠物互动、退出登录二次确认对话框 + 宠物挽留动画 |
...
| `ValidateStage.tsx` | **验证阶段**：三列看板（待验证/进行中/已验证，DnD 拖拽），验证项（标题/描述/结果录入），验证工具箱（问卷/访谈/社区/竞品），GO/NO-GO 决策系统，AI 智能建议弹窗 |
| `ValidateSuggestModal.tsx` | 验证阶段 AI 建议弹窗：展示 AI 生成的验证项和验证工具建议，支持 append（追加）/ overwrite（覆盖）两种应用模式 |
...
| `PixelPet.tsx` | **像素宠物动画组件**：8-bit 像素风格渲染，支持 idle/blink/happy/celebrate 四种动画状态，可配置缩放比例，集成于 AIChat 和 ProjectBoard |
| `PixelPet.frames.ts` | 像素宠物帧数据定义 + `PIXEL_PET_CATALOG`：猫/赛博猫/狗/龙/兔等像素画帧序列 |
| `DotGridBackground.tsx` | 登录/着陆页背景网格动效：Canvas 2D + 弹簧物理模型，鼠标移动产生斥力涟漪/脉冲交互，输入框聚焦触发焦点涟漪，移开弹性回位，支持暗色/亮色主题 |
| `ModelSelector.tsx` / `ModelSelectorModal.tsx` | 模型选择器按钮 + 弹窗，从后端拉取可用提供商列表（含 Ollama），同步用户首选模型 |
...
| 文件 | 功能 |
|------|------|
| `LoginModal.tsx` | 认证弹窗（全屏覆盖）：网格点背景 + 微动画，三标签页（登录/注册/找回密码），密码强度实时指示器，用户名/密码表单 + 邮箱注册 + 找回密码，Google/GitHub OAuth 按钮，JWT 存储到 localStorage |
| `GitHubImportModal.tsx` | **GitHub 仓库导入**：OAuth `public_repo` 授权连接 GitHub 账号，选择公开仓库，AI 自动分析 README/metadata 建议阶段并预填项目字段。三步流程：仓库选择 → AI 预览 → 项目创建 |
| `AIConfigModal.tsx` | AI 配置弹窗：两栏自适应布局（左侧提供商选择，右侧表单输入），API Key/Base URL/Model 输入，保存并测试 |
| `ThemeSwitcher.tsx` | 主题切换按钮（Sun/Moon） |
| `LanguageSwitcher.tsx` | 语言切换下拉面板（7 语言：中/日/韩/西/法/德/英），统一放置到所有页面 |
...
| `ProfilePage.tsx` | 用户个人资料独立页面：头像/昵称/邮箱展示、密码修改、OAuth 账号绑定与解绑、账户注销 |
...
| 文件 | 功能 |
|------|------|
| `app/models.py` | SQLAlchemy 模型：User, Project, Stage, PromoteTask, PromoteSuggestion, AIConfig, AICallLog, OperationLog, **AgentRun, AgentTask**（多 Agent 并行任务跟踪）；枚举：ProjectStatus, StageKey, AIProvider, UserRole |
...
| 文件 | 功能 |
|------|------|
| `auth.py` | `/auth`：POST /login（返回 JWT + Token Version 绑定），POST /logout（刷新 Token Version 使旧 Token 全局失效），POST /change-password（改密后强制重新登录），GET /me，GET/PUT /preferred-model，POST /register（邮箱注册 + honeypot 反机器人 + 速率限制），GET /verify-email，POST /forgot-password，POST /reset-password，GET /oauth/google（State 一次性校验 + 409 冲突保护），GET /oauth/github（同上）。Refresh Token Rotation：每次刷新返回全新 token pair |
...
| `ai.py` | `/ai`：GET /ping，GET /providers（含 Ollama 本地模型），GET /configs（admin 权限），PUT /configs/{provider}（admin 权限），POST /test/{provider}（admin 权限），**POST /chat（核心 SSE 流式聊天）**——支持阶段上下文注入、阶段原生格式验证（【阶段事实】【关键缺口】【下一步动作】【可同步JSON】）、提取 sync_payload/next_question，GET /stage-context，POST /promote-suggest（项目所有权校验），GET /call-logs |
...
| 文件 | 功能 |
|------|------|
| `ai_proxy.py` | AIProxyService：5 个提供商（DeepSeek / Kimi / 豆包 / OpenAI / Ollama），get_active_config，decrypt_api_key，test_connection，chat_completion（异步生成器，httpx 60 秒超时，follow_redirects=False，SSE 错误处理，记录 AICallLog），generate_promote_suggestions（LRU 有界缓存 128），init_default_ai_configs |
...
### 8.5 当前项目状态速览（截至 2026-04-29）

根据历史文档和代码审计：

- **完全实现**（约 80%）：六阶段基础编辑器、AI 聊天（SSE 真流式 + TTFT 优化）、用户认证（邮箱注册 + Google/GitHub OAuth + 找回密码 + 注册限流 + honeypot 反机器人 + Token Version + Refresh Token Rotation + OAuth State 一次性校验）、国际化（7 语言）、主题切换、项目蓝图、IdeaStage 拖拽、AIPetConfig、PixelPet 像素宠物、ValidateSuggestModal、MonthView、ImageUpload、Stripe 测试支付、GitHub Project Import、项目标题编辑、Docker 容器化部署、Ollama 本地 AI、前端性能全面优化（懒加载/节流/memo/API 缓存）、DotGridBackground 涟漪动效（含脉冲交互 + 焦点集成）、ProfilePage 个人资料、landing/ Next.js 宣传站、安全审计 P0+P1 全量修复、**CI/CD 自动部署（GitHub Actions → 阿里云 Docker）**、**混合云部署配置（Vercel 前端 + Zeabur 后端）**、**域名体系（wanchun.me 子域名：app / api / sparkbin）**、SSL 快速配置脚本、Resend 邮件服务（验证/重置）、AI 配置自动从环境变量同步（零手动配置）、登录安全增强（IP 级 rate-limit + honeypot）、README 重构（Banner + Mermaid 架构图 + 录制指南）
- **部分实现**（约 15%）：ValidateStage 跨列拖拽（复杂度高）、闭环反馈系统、月视图直接编辑、多 Agent 并行任务系统（模型层已完成，API/前端待实现）、**支付系统（方案已定，Stripe 测试模式代码 100% 完成，待切换 Live Mode 上线）**
- **未实现**（约 5%）：A/B 测试、自动归类反馈、反馈回流到原型的完整闭环、项目蓝图力导向图谱、多平台一键发布/配图生成、订阅制（Pro/Team tier）
