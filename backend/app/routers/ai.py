...
# 内存中的 Chat Job 存储（生产环境应使用 Redis）
...
async def chat_sync(
    ...
    """
    同步 AI 聊天接口（小程序专用）
    直接返回完整 AI 回复，无 SSE 流式输出
    """
    _check_ai_quota(current_user, db)
    _deduct_ai_credit(current_user, db, reference_id="chat-sync")
    ai_service = AIProxyService(db, user_id=str(current_user.id))
        ...
        if project:
            ...
            if stage:
                ...
                merged_messages = [
                    {
                        "role": "system",
                        "content": build_stage_native_system_prompt(stage_snapshot, True),
                    },
                    *merged_messages,
                ]
    ...
    try:
        ...
        # 非流式模式下，generator 是一个 async generator，但只 yield 一次完整结果
        full_text = ""
        async for chunk in generator:
            for line in chunk.splitlines():
                stripped = line.strip()
        ...
        return {
            ...
            "tokens_used": len(full_text) // 2,  # 粗略估算
        }
    except Exception:
        raise HTTPException(
...
async def create_chat_job(
    ...
    """
    创建 AI 聊天异步任务（小程序轮询接口）
    返回 job_id，前端通过 GET /chat-job/{job_id} 轮询结果
    """
    _check_ai_quota(current_user, db)
    _deduct_ai_credit(current_user, db, reference_id="chat-job")

    job_id = f"chat_{uuid.uuid4().hex}"
...
async def _run_chat_job(
    ...
    try:
        ...
        ai_service = AIProxyService(db, user_id=user_id)
        merged_messages = list(messages)

        if project_id and stage_key:
            project = db.query(Project).filter(
        ...
        async for chunk in generator:
            for line in chunk.splitlines():
                ...
                if text:
                    full_text += text
                    job["partial_content"] = full_text.strip()

        job["status"] = "completed"
        job["content"] = full_text.strip()
...
@router.get("/chat-job/{job_id}", response_model=ChatJobStatusResponse)
def get_chat_job(
    job_id: str,
    current_user: User = Depends(get_current_user)
):
